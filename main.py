...
def main(args):
    ...
    if args.dataset == 'ISRUC':
        fold_clean, fold_noise, fold_contaminated, fold_len = get_isruc(args.data_dir, args.noise_type)

    if args.do_train:
        
        DataGenerator = kFoldGenerator(fold_clean, fold_noise, fold_contaminated, args.batch_size , args.noise_type)
...
def evaluate(
    ...
    with torch.no_grad():
        val_start_time = time()
        for noiseeeg_batch, indicator, cleaneeg_batch in dataloader:
            noiseeeg_batch = noiseeeg_batch.to(DEVICE)
            indicator = indicator.to(DEVICE)
            cleaneeg_batch = cleaneeg_batch.to(DEVICE)
