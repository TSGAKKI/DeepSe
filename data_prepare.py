...
class kFoldGenerator():
    ...
    # Initializate
    def __init__(self, fold_clean, fold_noise, fold_contaminated, batch_size, noise_type ):
    ...
    def getFold(self, i):
        ...
        for p in range(self.k):
            if p != i:
                if isFirst:
                    ...
                    train_noise = np.concatenate((train_noise, self.noise_ori[p]))
                    
            else:
                val_clean = self.clean_list[p]
                val_contaminated = self.noise_list[p]

        train_contaiminated = train_contaiminated.reshape(-1,train_contaiminated.shape[-1])
        train_noise = train_noise.reshape(-1,train_noise.shape[-1])
        train_clean = train_clean.reshape(-1,train_clean.shape[-1])

        val_clean = val_clean.reshape(-1,val_clean.shape[-1])
        val_contaminated = val_contaminated.reshape(-1,val_contaminated.shape[-1])

        train_input = np.concatenate((train_contaiminated, train_noise, train_clean), axis=0)
        train_clean_total = np.concatenate((train_clean, train_noise, train_clean), axis=0)

        train_indicator1 = np.zeros(train_contaiminated.shape[0])
        train_indicator2 = np.ones(train_noise.shape[0])
        train_indicator3 = np.zeros(train_clean.shape[0])
        train_indicator = np.concatenate((train_indicator1, train_indicator2, train_indicator3), axis=0)
        
        train_clean_total = torch.from_numpy(train_clean_total).type(torch.FloatTensor)  
        train_input = torch.from_numpy(train_input).type(torch.FloatTensor)
        train_indicator = torch.from_numpy(train_indicator).type(torch.FloatTensor)
        train_indicator = train_indicator.unsqueeze(1)
        
        train_dataset = torch.utils.data.TensorDataset(train_input, train_indicator, train_clean_total)
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True)

        val_indicator = np.zeros(val_clean.shape[0])
        ...
        val_indicator = torch.from_numpy(val_indicator).type(torch.FloatTensor)
        val_indicator = val_indicator.unsqueeze(1)

        if self.noise_type == 'clean':
            val_dataset = torch.utils.data.TensorDataset(val_clean, val_clean)
        else:
            val_dataset = torch.utils.data.TensorDataset(val_contaminated, val_indicator, val_clean)
